...
This repository contains the analysis code and experimental logs for the **VUV (Vacuum Ultraviolet) monochromator system** equipped with a **deuterium lamp** and **SiPM (Silicon Photomultiplier)** photodetectors. 
...
* **Theoretical Modeling:** A Python script (`deuterium\_test2.py`) was developed to combine:
  1. The raw emission spectrum from the lamp's datasheet (interpolated red curve).
...
* **Excellent Agreement:** The calibrated experimental spectrum and the theoretical model overlaid perfectly in both linear and logarithmic scales.
* **Low-Wavelength Cutoff:** The complete absence of signal below 180–190 nm confirmed the intense absorption of air, validating the absolute necessity of vacuum conditions to explore the VUV range.
...
* **Objective:** Obtain the unattenuated deuterium lamp spectrum and map the actuator steps precisely to physical wavelengths.
* **Method:** A 5-hour *long run* with a fine step size of 0.01° was recorded from -15° to 15°. A minimization procedure (Chi² or fitting on the primary peak at ~160 nm) was implemented.
...
* **Hardware Warning:** Mechanical actuator faults ("position not reached") occurred consistently between [-1° or 1°] and 2.6°. Data within this specific window should be treated with caution.
...
* **Method:** Fine scans (0.01° steps) were run over restricted peak zones (e.g., 2.5° to 5°).
  * **Test 1:** Fixed exit slit (0.01 mm), varying entrance slit (0.01 mm to 5 mm).
...
* **SiPM Stabilization Study:** 
  ...
  * **Power Cycling Effect:** Between each of these short runs, the SiPM was completely powered off and then powered back on via the `initialise\_instrument` function. The resulting data showed a distinct downward curvature at the start of each measurement.
  * **Conclusion:** This artifact strongly indicates that the **SiPM itself requires a dedicated stabilization period** upon power-up. Once a slightly longer run was recorded without cycling the power, the curve flattened out and perfectly mirrored the stable long-term trend observed the previous day.

---

## Conclusions and Best Practices

Based on these intensive testing phases, the following guidelines are established for upcoming runs and the parallel *Metalens* project:

1. **Warm-up Protocol:** Always turn on the system (computer, actuator, lamp, and SiPM) **at least 20 to 30 minutes before** starting data acquisition to eliminate initial thermal and power-cycling drift.
...
4. **Hardware Validation:** Stage milestones and Trivio dates are fully updated. Next steps include validating the mechanical integration of the 3x3 SiPM array on the PCB and testing the 3D-printed metalens supports designed by Brais.# Deuterium Lamp Spectrum, Calibration, and Stability Studies
...
### Methodology
* **Acquisition:** A short run (angular scan from -15° to 15°) was performed at atmospheric pressure.
* **Theoretical Modeling:** A Python script (`deuterium_test2.py`) was developed to combine:
  1. The raw emission spectrum from the lamp's datasheet (interpolated red curve).
  2. The Photon Detection Efficiency (PDE) of the SiPM (simplified to a constant value of `0.24`).
  3. Atmospheric absorption along the optical path ($L \approx 0.7\text{ m}$) using the transmission equation:
     $$T(\lambda) = \exp\left(-\frac{\text{PATH\_LENGTH}}{L_{\text{atten}}(\lambda)}\right)$$
...
* **SiPM Stabilization Study:** 
  * Short-duration baseline tests were conducted using the same script. 
  * **Power Cycling Effect:** Between each of these short runs, the SiPM was completely powered off and then powered back on via the `initialise_instrument` function. The resulting data showed a distinct downward curvature at the start of each measurement.
  * **Conclusion:** This artifact strongly indicates that the **SiPM itself requires a dedicated stabilization period** upon power-up. Once a slightly longer run was recorded without cycling the power, the curve flattened out and perfectly mirrored the stable long-term trend observed the previous day.
...
2. **Actuator Management:** Avoid repeated wide-range angular scans over short intervals to prevent motor overheating. Prioritize targeted scans around specific peaks of interest.
3. **Optimal Optical Setup:** Set both slits to **0.05 mm** to maximize resolution without severely sacrificing photocurrent intensity.
4. **Hardware Validation:** Stage milestones and Trivio dates are fully updated. Next steps include validating the mechanical integration of the 3x3 SiPM array on the PCB and testing the 3D-printed metalens supports designed by Brais.
